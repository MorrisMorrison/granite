...
| Accent | `--accent #3b82f6` · `--accent-hover` · `--accent-text #fff` · `--accent-subtle` |
...
Built **as they're first applied** to a screen (no unused code), shadcn-flavored:
`Icon` (inline SVG set) · `TabBar` (bottom nav) · `Button` (primary/secondary/outline/ghost/destructive,
sizes, block, loading, icon, href) · `IconButton` · `Card` · `Field` (label + input/select/textarea +
error) · `ListRow` (tappable row: title/subtitle/trailing) · `Badge` · `EmptyState` · `Sheet` (bottom
sheet for the exercise picker).
...
4. **Polish + new screens** — motion pass, then routine folders / exercise create-edit / settings.

Each slice is verified offline against the `:8080` production build, with `data-testid`s in place as screens
are built. The UI e2e suite follows the overhaul.
