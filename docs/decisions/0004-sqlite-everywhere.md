# ADR-0004 — SQLite on device and server
...
file. Server storage is **SQLite, concretely** — queries are generated by [sqlc](https://sqlc.dev)
against the SQLite schema, and core sync behaviour (the monotonic `server_seq` cursor) lives in SQLite
`AFTER INSERT/UPDATE` triggers. There is **no storage-abstraction interface** and no other backend; we
don't pre-build one. Porting to another engine (e.g. PostgreSQL, if someone ever needed a large shared
instance) would mean rewriting the migrations, the triggers, and regenerating the sqlc layer — a
deliberate, one-time effort undertaken only if that need actually arrives, not a seam maintained
speculatively.
...
- **PostgreSQL on the server.** The "default" choice, but it adds a second process/container to run,
  ...
  Rejected as the storage engine; not kept as a parallel option either (see Decision).
...
- ➖ Not suited to a large multi-writer shared instance. That case is out of scope for the target
  use case; if it ever arrived it would be a deliberate port (rewrite migrations + triggers, regenerate
  sqlc), not a config switch — and we don't carry the abstraction cost to keep that door open today.
